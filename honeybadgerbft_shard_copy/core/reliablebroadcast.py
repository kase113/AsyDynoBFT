...
def reliablebroadcast(sid, pid, N, f, leader, input, receive, send, start, end, max):
    ...
    if pid == leader:
        ...
        for i in range(start, end):
            branch = getMerkleBranch(i%N, mt)
            send(i, ('VAL', roothash, branch, stripes[i%N]))

        # send(5,('RTEST',time.time(),pid))
    ...
    while True:  # main receive loop
        ...
        sender, msg = receive()
        shard_sender = sender % N
        # print('sender and msg', sender, msg[0],shard_sender)
        if msg[0] == 'VAL' and fromLeader is None:
            ...
            if sender != leader:
                ...
                continue
            try:
                assert merkleVerify(N, stripe, roothash, branch, pid%N)
            ...
            (_, roothash, branch, stripe) = msg
            # Validation
            if roothash in stripes and stripes[roothash][shard_sender] is not None \
               or shard_sender in echoSenders:
                print("Redundant ECHO")
                continue
            try:
                assert merkleVerify(N, stripe, roothash, branch, shard_sender)
            except AssertionError as e:
                print("Failed to validate ECHO message:", e)
                continue

            # Update
            stripes[roothash][shard_sender] = stripe
            echoSenders.add(shard_sender)
            ...
            if shard_sender in ready[roothash] or shard_sender in readySenders:
                print("ready[roothash] and readSenders:", ready[roothash], readySenders)
                print("Redundant READY")
                continue

            # Update
            ready[roothash].add(shard_sender)
            readySenders.add(shard_sender)
