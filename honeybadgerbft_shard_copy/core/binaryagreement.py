...
def binaryagreement(sid, pid, N, f, coin, input, decide, receive, send, start, end):
    ...
    def _recv():
        while True:  # not finished[pid]:
            ...
            assert shard_sender in range(N)
            if msg[0] == 'EST':
                # BV_Broadcast message
                _, r, v = msg
                assert v in (0, 1)
                if shard_sender in est_values[r][v]:
                ...
                est_values[r][v].add(shard_sender)
                ...
                _, r, v = msg
                assert v in (0, 1)
                if shard_sender in aux_values[r][v]:
                    # FIXME: raise or continue? For now will raise just
                    # because it appeared first, but maybe the protocol simply
                    # needs to continue.
                ...
                aux_values[r][v].add(shard_sender)
                logger.debug(
    ...
    while True:  # Unbounded number of rounds
        ...
        try:
            est, already_decided, maj = set_new_estimate(
                values=values,
                maj=maj,
                s=s,
...
def set_new_estimate(*, values, maj, s, already_decided, decide):
    if len(values) == 1:
        v = next(iter(values))
        est = v
        if v == s:
        ...
        est = 2
    elif values == {1, 2} or values == {0, 2}:
        est = v
    else:
        est = s

    if values.intersection({2}) == {2}:
        maj = False

    elif maj == False:
        maj = True
    
    return est, already_decided, maj
