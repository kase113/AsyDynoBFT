...
from honeybadgerbft_shard.core.acrossshardcast import crossshardbroadcast
...
class HoneyBadgerBFT():
    ...
    def __init__(self, sid, pid, B, N, f, sPK, sSK, ePK, eSK, send, recv, K, R, MR, logger, mute=False):
        self.sid = sid
        self.id = pid 
        self.B = B
        self.N = N
    ...
    def _run_round(self, r, tx_to_send, send, recv):
        ...
        arbc_outputs = [Queue(1) for _ in range(8)]
        # for j in range(N):
        for j in range(4, 8):
            def crbc_send(k, o):
                send(k, ('CROSS_RBC', j, o))
            arbc_input = 'this is the test of arcb'
            arbc = gevent.spawn(crossshardbroadcast, sid, pid, N, f, j, arbc_input, crbc_recvs[j].get, crbc_send, self.logger)
            arbc_outputs[j] = arbc.get
            arbc_recv = gevent.spawn(broadcast_receiver_loop, recv, recv_queues)

        # for j in range(4, 8):
        #     def crbc_send(k, o):
        #         send(k, ('CROSS_RBC', j, o))
        #     arbc_input = 'this is the test of arcb'
        #     arbc = gevent.spawn(crossshardbroadcast, sid, pid, N, f, j, arbc_input, crbc_recvs[j].get, crbc_send)

        arbc_recv.join(timeout=0.05)
        self.logger.info('arbc has done')
