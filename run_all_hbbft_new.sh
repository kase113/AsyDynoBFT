...
while [ "$i" -lt 7 ]; do
  # batchsize=1000*i
  batchsize=$((1000 * i))
  sh ./run_socket_network_start_new.sh hbbft 8 2 $batchsize 2 0
  while true; do
...
while [ "$i" -lt 7 ]; do
  # batchsize=1000*i
  batchsize=$((1000 * i))
  sh ./run_socket_network_start_new.sh hbbft 16 4 $batchsize 2 0
  while true; do
...
while [ "$i" -lt 7 ]; do
  # batchsize=1000*i
  batchsize=$((1000 * i))
  sh ./run_socket_network_start_new.sh hbbft 32 8 $batchsize 2 0
  while true; do
...
while [ "$i" -lt 7 ]; do
  # batchsize=1000*i
  batchsize=$((1000 * i))
  sh ./run_socket_network_start_new.sh hbbft 36 9 $batchsize 2 0
  while true; do
    echo "---- 开始扫描容器运行情况，hbbft，N=36 f=9 B=$batchsize K=2 ----"
    # 2.2 判断容器数量，如果不是8个，退出
    if [ $(docker ps -a | grep "bft" | wc -l) -eq 0 ]; then
      echo "---- 成功运行hbbft，N=32 f=8 B=$batchsize K=2 ----"
      break
      ...
      sleep 30
      if [ $(docker ps -a | grep "bft" | wc -l) -lt 20 ]; then
        echo "---- 成功运行hbbft，N=36 f=9 B=$batchsize K=2 ----"
        docker rm -f $(docker ps -a | grep "bft:latest" | awk '{print $1}')
        break
      fi
    fi
