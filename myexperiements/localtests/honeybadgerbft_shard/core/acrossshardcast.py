...
def crossshardbroadcast (sid, pid, N, f, leader, input, receive, send, logger):
    ...
    if logger != None: 
        logger.info('run it4')
        logger.info('run it4', sid)

    if pid == leader:
        transaction = input
        for i in range(N):
            send(i, ('AREADY', transaction))

    def decide(transaction, readySender):
        end_time = time.time()
        print('decide:',transaction,'this is the ready_pid:', str(readySender))
    ...
    while True:
        ...
        if msg[0] == 'AREADY':
            ...
            if sender in readySenders:
                print("Redundant AREADY")
                continue

            #Update
            # ready[transaction].add(sender)
            readySenders.add(sender)
            print('len ready sender:', len(readySenders))
            
            if len(readySenders) >= OutputThreshold:
                return decide(transaction, readySenders)
