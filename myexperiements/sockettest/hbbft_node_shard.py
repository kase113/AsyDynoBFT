...
# def load_key(id, R, shard,N):
#     key_filepath = '/keys-' + str(R) + '-' + str(shard) + '/'

#     with open(os.getcwd() + key_filepath + 'sPK.key', 'rb') as fp:
#         sPK = pickle.load(fp)

#     with open(os.getcwd() + key_filepath + 'ePK.key', 'rb') as fp:
#         ePK = pickle.load(fp)

#     with open(os.getcwd() + key_filepath + 'sSK-' + str(id%N) + '.key', 'rb') as fp:
#         sSK = pickle.load(fp)

#     with open(os.getcwd() + key_filepath + 'eSK-' + str(id%N) + '.key', 'rb') as fp:
#         eSK = pickle.load(fp)
...
class HoneyBadgerBFTNode_shard (HoneyBadgerBFT):
 
    def __init__(self, sid, id, B, N, f, bft_from_server, bft_to_client,ready: mpValue, stop: mpValue, K=3, R=1, MR=1, per=1, mode='debug', mute=False,debug=False, bft_running: mpValue=mpValue(c_bool, False), tx_buffer=None):
        ...
        # self.per = per
        self.per = 0.01
        HoneyBadgerBFT.__init__(self, sid, id, B, N, f, self.sPK, self.sSK, self.ePK, self.eSK, send=self.send, recv=self.recv, shard=self.shard, K=K, R=R, MR=MR, logger=set_consensus_log(id), mute=mute)


        # for i in range(R-1):
        #     print('this is the i', i, self.shard)
        #     if (id >=i * self.shard and id <= (i+1) * self.shard):
        #         print('this is the after id:', id)
        #         self.instance[i] = HoneyBadgerBFT.__init__(self, sid, id, B, N, f, self.sPK, self.sSK, self.ePK, self.eSK, send=self.send, recv=self.recv, K=K, logger=set_consensus_log(id), mute=mute)
        #     else:
        #         pass
    ...
    def run(self):
        ...
        self.run_bft()
        trans = self.get_tx()
        trans_splice = int(self.per * len(trans))
        # self.ac_trans = trans[:trans_splice]
        self.ac_trans = str(trans[:1])


def main(sid, i, B, N, f, addresses, K):
    badger = HoneyBadgerBFTNode_shard(sid, i, B, N, f, addresses, K)
