...
def instantiate_bft_node(sid, i, B, N, f, K, R, S, T, bft_from_server: Callable, bft_to_client: Callable, ready: mpValue,
                         stop: mpValue, protocol="hbbft_shard", mute=False, F=100, debug=False, omitfast=False, bft_running: mpValue=mpValue(c_bool, False)):
    bft = None
    if protocol == 'dumbo':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--P', metavar='P', required=False,
                        help='protocol to execute', type=str, default="hbbft_shard")
    ...
    try:
        with open('hosts.config', 'r') as hosts:
            ...
            # for address in lines[start:end]:
