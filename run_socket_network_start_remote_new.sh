...
beginNum=${12}
endNum=${13}
echo "---- 当前hosts1: $hosts1, hosts2: $hosts2, hosts3: $hosts3, hosts1endNum: $hosts1endNum, hosts2endNum: $hosts2endNum, beginNum: $beginNum, endNum: $endNum ----"
...
if [ $beginNum -eq 0 ]; then
  ...
  while [ "$i" -lt $N ]; do
    echo $i $hosts3 $hosts3 $((20000 + $((500 * $i)))) >>hosts.config
    echo "---- 写入第【$i】个节点的hosts.config文件：$i $hosts3 $hosts3 $((20000 + $((500 * $i)))) ----"
    i=$((i + 1))
  done
#  # 3.4.远程拷贝密钥到其他服务器
#  scp /kolla/yaozc/lzh/project/dumbo_1029/hosts.config root@172.22.110.18:/kolla/yaozc/lzh/project/dumbo_1029/hosts.config
fi
...
if [ $beginNum -eq 0 ]; then
  ...
  rm -rf /kolla/yaozc/lzh/data/keys
  mkdir -p /kolla/yaozc/lzh/data/keys
  ...
  echo "---- 成功创建key-gen网桥 ----"
  # 3.3.启动key-gen容器
  docker run --rm -itd --name trusted-key --network=key-network -v /kolla/yaozc/lzh/data/keys:/home/keys $IMAGE_NAME:$IMAGE_TAG /bin/bash -c "python3 run_trusted_key_gen.py --N $N --f $f"
  echo "---- 启动key-gen容器，生成密钥 ----"
#  # 3.4.远程拷贝密钥到其他服务器
#  scp -r /kolla/yaozc/lzh/data/keys root@172.22.110.18:/kolla/yaozc/lzh/data
fi

# 5.启动N个容器，i为容器编号，j为网桥编号
echo "---- 开始循环启动容器 ----"
mkdir -p /kolla/yaozc/lzh/data/logs/$IMAGE_NAME/$P/$N-$f-$B-$K
...
while [ "$i" -lt $((endNum + 1)) ]; do
  ...
  if [ $MR -eq 0 ]; then
    docker run --cpus=2 -m 4096m --rm -itd --name $IMAGE_NAME-$i --net=host \
      -v /kolla/yaozc/lzh/data/keys:/home/keys \
      -v /kolla/yaozc/lzh/data/logs/$IMAGE_NAME/$P/$N-$f-$B-$K:/home/log \
